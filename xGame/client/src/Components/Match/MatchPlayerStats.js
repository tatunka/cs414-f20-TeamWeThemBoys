...
import {getPieceList, showTable, createBoardPiece} from './HelpfulMatchTools.js'
...
const MatchPlayerStats = (props) => {
    ...
    const determineTaken = () => {
        let totalPieces = '';
        if(opponent){
            totalPieces = (activeColor === 'white') ? getPieceList('white') : getPieceList('black');
        }else{
            totalPieces = (activeColor === 'white') ? getPieceList('black') : getPieceList('white');
        }
    ...
    const generateDisplay = () => {
        ...
        for(let i = 0; i < takenPieces.length; i++){
            tempDisplay[rowCount].push(
                createBoardPiece(takenPieces[i],'hidden', activeColor + "Captured" + i, null)
            );
            if(i % 5 === 0){
                tempDisplay.push([]);
                rowCount++
    ...
    return(
        <div className={'sidebar'}>
            ...
            {displayTurnCounter()}
            <Grid container className={'gameBoard'} spacing={0}>{showTable(generateDisplay())}</Grid>
        </div>
    )
}
